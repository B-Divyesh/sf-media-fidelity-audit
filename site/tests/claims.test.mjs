...
test('@claim:read-scope does not scan media outside the chosen source and archive folders', () => {
  const root = temp('scope'); const source = join(root, 'source'); const archive = join(root, 'archive'); const elsewhere = join(root, 'elsewhere'); mkdirSync(source); mkdirSync(archive); mkdirSync(elsewhere); writeFileSync(join(source, 'photo.jpg'), 'same'); writeFileSync(join(archive, 'photo.jpg'), 'same'); writeFileSync(join(elsewhere, 'private.jpg'), 'must not be scanned'); const before = hash(join(elsewhere, 'private.jpg')); const output = join(root, 'manifest.json'); assert.equal(audit(source, archive, output).status, 0); const manifest = JSON.parse(readFileSync(output)); assert.equal(JSON.stringify(manifest).includes('private.jpg'), false); assert.equal(hash(join(elsewhere, 'private.jpg')), before); rmSync(root, { recursive: true });
});
